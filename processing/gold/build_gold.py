...
def run_gold_pipeline(spark, date_path: str):
    ...
    try:
        # 1. Read unified silver
        df_silver = read_silver(spark, date_path)
        ...
        write_gold(
            df_jobs_summary,
            f"s3a://data-lake/gold/jobs_summary/dt={date_path}"
        )

        # 6. Write salary analysis
        write_gold(
            df_salary_analysis,
            f"s3a://data-lake/gold/salary_analysis/dt={date_path}"
        )

        # 7. Write jobs detail
        write_gold(
            df_jobs_detail,
            f"s3a://data-lake/gold/jobs_detail/dt={date_path}"
        )
